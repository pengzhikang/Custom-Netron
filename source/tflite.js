    ...
    open(context, match) {
        return context.require('./tflite-schema').then(() => {
            ...
            return tflite.Metadata.open(context).then((metadata) => {
                // metadata is about metadata.json api
                // metadata = tflite.Metadata._metadata = new tflite.Metadata(data)
...
tflite.Model = class {
    ...
    constructor(metadata, model) {
        ...
        // find custom operators in model file
    ...
    /* example about get function(){}
    myobject = {
    get pzk(){
        return this.pzk.toUpperCase();
    }
    }
    m1 = new myobject();
    m1.pzk = "pengzhikang";
    console.log(m1.pzk);// console will print "PENGZHIKANG", because it will exec pzk() when get m1.pzk
    */
    get format() {
        return this._format;
    }
...
tflite.Metadata = class {
    ...
    static open(context) {
        ...
        return context.request('tflite-metadata.json', 'utf-8', null).then((data) => {
            ...
            // when success load metadata.json, return tflite.Metadata._metadata
...
// static open is the same as constructor
// subclass is binaryreader
